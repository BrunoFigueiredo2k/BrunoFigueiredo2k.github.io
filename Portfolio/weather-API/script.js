...
function changeCity() {
    ...
    $.getJSON("http://api.openweathermap.org/data/2.5/weather?q=" + city + "&APPID=24009eccc8c9d83d6bcb4bf23c273a08&units=metric", function (data) {
        ...
        if (icon.includes("d")) {
            cardDiv.style.backgroundImage = "url('../weather-API/img-weather/" + status.toLowerCase() + ".jpg')";
        } else if (icon.includes("n")) {
            cardDiv.style.backgroundImage = "url('../weather-API/img-weather/night" + status.toLowerCase() + ".jpg')";
        }
